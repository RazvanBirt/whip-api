...
import { create, getAll, getById, update, remove } from "./bodyTypes.service";
...
export const createBodyType: RequestHandler = async (req, res) => {
  const body = req.body;
...
export const getBodyTypes: RequestHandler = async (req, res) => {
  const search =
  ...
  const sortField =
    req.query.sortField === "country" ? "country" : "name";
  const sortOrder =
...
export const getBodyType: RequestHandler<{ id: string }> = async (req, res) => {
  const { id } = req.params;
...
export const updateBodyType: RequestHandler<{ id: string }> = async (req, res) => {
  const { id } = req.params;
  const body = req.body;
...
export const deleteBodyTypes: RequestHandler = async (req, res) => {
  const body = req.body;
  ...
  try {
    ...
    const paths = makes.map((m) => m.imagePath).filter((p): p is string => !!p);
