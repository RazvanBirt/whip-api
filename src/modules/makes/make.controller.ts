...
export const createMakeWithImage: RequestHandler = async (req: any, res: any) => {
    ...
    const guard = Guard.againstNullOrUndefinedBulk([
        { argument: body?.name, argumentName: "Name" },
        { argument: body?.country, argumentName: "Country" },
    ]);
    if (!guard.succeeded) return res.status(400).json({ error: "Missing required field", field: guard.argumentName });

    if (!file) return badRequest(res, 'Missing file field "image"');
    if (!file.mimetype.startsWith("image/")) return badRequest(res, "Only image files are allowed");

    try {
        // 1) create make
        const createdRes = await createOne({ name: body.name, country: body.country });
        if (!createdRes.success) return badRequest(res, createdRes.error);

        const make = createdRes.created;

        // 2) upload to supabase
        const ext = (file.originalname.split(".").pop() || "jpg").toLowerCase();
        const objectPath = `makes/${make.id}-${Date.now()}.${ext}`;
        ...
        if (uploadError) return serverError(res, uploadError);

        const { data } = supabaseAdmin.storage.from(BUCKET).getPublicUrl(objectPath);
        const imageUrl = data.publicUrl;

        // 3) update make with image url
        const updated = await prisma.make.update({
            where: { id: make.id },
            data: { imageURL: imageUrl, imagePath: objectPath },
            select: { id: true, name: true, country: true, imageURL: true, imagePath: true },
        });

        return success(res, { make: updated });
    } catch (err) {
        return serverError(res, err);
    }
};
